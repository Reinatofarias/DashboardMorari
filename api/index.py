...
def fetch_data(start=None, end=None):
    try: cfg = load_cfg()
    except: return None, "Config nao encontrado"
    token = cfg.get("facebook",{}).get("access_token","")
    acid = cfg.get("facebook",{}).get("ad_account_id","")
    if not token or not acid: return None, "Token/Account nao config"
    if not token_valid(token): return None, "Token invalido"
    ...
    if start and end: p["time_range"] = json.dumps({"since":start,"until":end})
    else: p["date_preset"] = "last_30d"
    try:
        r = requests.get(url, params=p, timeout=30)
        d = r.json()
        if "error" in d: return None, d['error'].get('message','Erro')
        processed = []
        for row in d.get("data",[]):
            m = calc_metrics(row)
            purchases = 0
            for a in row.get("actions",[]):
                if a.get("action_type")=="purchase": purchases=int(float(a.get("value",0)))
...
def get_config():
    try:
        c = load_cfg()
        return jsonify({'configured': bool(c.get('facebook',{}).get('access_token')), 'expires_at': c.get('token',{}).get('expires_at')})
...
def update():
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': f'{len(data)} regs!'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})
